...
public class SearchEngineServlet extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		...
		try {
			...
			Object homeObject = context.lookup("java:comp/env/ejb/SearchEngineBeta");
					...
					if ((setStart + resultsPerPage) < numberOfResults)
						out.println("<a href=\""
								+ response.encodeURL(request.getContextPath()
										...
										+ filterString + "\" >Next</a>");
					out.println("<BR>");
					out.println("<input type= 'submit' name='DownLoad' value='Download'>" );
					out.println("</form>");
	...
	throws ServletException, IOException {
		...
		try {
			...
			String download = request.getParameter("DownLoad");
			count = 0 ;
			for (int jj=0; jj<resultsPerPage; jj++){
				String checkValue = request.getParameter("check"+jj);
				System.out.println(" checkvalue is : " + checkValue);
				if (checkValue != null)
					count ++ ;
			}
			...
			if (advanced != null && !advanced.equals(""))
				...
				printAdvanced(request, response);
			else if (download != null && !download.equals("") && count >0)
			{
				...
				//Form header
				out.print("<form name=\"submitform\"action=\" " + response.encodeURL(request.getContextPath()	+ request.getServletPath())	+ "\" method=\"POST\">");
				...
				if (query.trim().length() > 0 && submit != null
					...
					if ( count <0){
						...
						+ count+ ">");
						out.println("<input type= 'submit' name='DownLoad' value='Download' onsubmit=\" return Click()\">" );
					}
					else 
					{
						validate = false;
						out.println("<input type = \"hidden\" name =\"count\" value="
								+ count+ ">");
						out.println("<input type= 'submit' name='DownLoad' value='Download' onclick=\"Click()\" >" );
	...
	String docroot = ApplicationSetup.getProperty("docroot","");
	...
	private void doDownload(HttpServletRequest request,
		...
		PrintWriter out = response.getWriter();
		String checkValue;
		ArrayList downloadables = new ArrayList();
		for (int i=0; i<resultsPerPage; i++){
			checkValue = request.getParameter("check"+i);
			if (checkValue != null){
				String tempPath = request.getParameter("images" + i);
				
				//out.println(tempPath+ "<BR>");
				System.out.println("path of the images to be downloaded " + tempPath);
				File tempInput = new File(tempPath);
				if (tempInput.exists())
				{
				downloadables.add(tempPath);
				}
			}
		}
		
		new File(docroot + "\\Zipfiles"  ).mkdir();
		String ZipPath = docroot + "\\Zipfiles\\test.zip";
		String pathToZipFile="/Zipfiles" + "\\test.zip";
		
		try
		{		
			System.out.println("Trying to write zipfile to " + ZipPath);
			FileOutputStream outStream = new FileOutputStream(ZipPath);
			ZipOutputStream zip = new ZipOutputStream(outStream);
			ZipEntry ze;
			String filename;
			Iterator files = downloadables.iterator();			
			while(files.hasNext())
			{
				filename = (String) files.next();	
				File tempInput = new File(filename);
				if (tempInput.exists()){
				FileInputStream inputStream = new FileInputStream(tempInput.getPath());				
				ze = new ZipEntry(filename);
				zip.putNextEntry(ze);
				byte[] buffer = new byte[2048];
				int leido;
					while(-1 != (leido = inputStream.read(buffer)))
					{						
		...
		//out.println("<a href=\""+ pathToZipFile +"\">Link to zip file:<BR></a>");
		//pathToZipFile = pathToZipFile.replaceAll("\\\\", "\\\\\\\\");
		out.println("<A HREF=\""+ pathToZipFile +"\" ; return false;\">download zip file</A>");
		//out.println("Click right and select 'Save as'");
	...
	private void printHTMLHeader(PrintWriter out) {
		...
		String src = "/common" + "\\Help.html";
		out
		.println("<table align = right font size=5 color = blue family=Arial><U></U><B></B><td bgcolor=white colspan=5><a href=\"" + src +"\" ><b>Help</b></a></td><td></table>");
