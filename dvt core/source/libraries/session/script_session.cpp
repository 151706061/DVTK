...
//<<===========================================================================
{
	...
	bool result = associationM.connect();
	if (!result)
	{
		if (loggerM_ptr)
		{
...
//<<===========================================================================
{
	...
	if (!result)
	{
		if (loggerM_ptr)
		{
			loggerM_ptr->text(LOG_ERROR, 1, "Can't send %s over TCP/IP to remote host %s on port number %d", WIDName(associateAc_ptr->getWidType()), getRemoteHostname(), getRemoteConnectPort());
		}
	}
...
//<<===========================================================================
{
	...
	if (!result)
	{
		if (loggerM_ptr)
		{
			loggerM_ptr->text(LOG_ERROR, 1, "Can't send %s over TCP/IP to remote host %s on port number %d", WIDName(associateRj_ptr->getWidType()), getRemoteHostname(), getRemoteConnectPort());
		}
	}
...
//<<===========================================================================
{
	...
	if (!result)
	{
		if (loggerM_ptr)
		{
			loggerM_ptr->text(LOG_ERROR, 1, "Can't send %s over TCP/IP to remote host %s on port number %d", WIDName(releaseRq_ptr->getWidType()), getRemoteHostname(), getRemoteConnectPort());
		}
	}
...
//<<===========================================================================
{
	...
	if (!result)
	{
		if (loggerM_ptr)
		{
			loggerM_ptr->text(LOG_ERROR, 1, "Can't send %s over TCP/IP to remote host %s on port number %d", WIDName(releaseRp_ptr->getWidType()), getRemoteHostname(), getRemoteConnectPort());
		}
	}
...
//<<===========================================================================
{
	...
	if (!result)
	{
		if (loggerM_ptr)
		{
			loggerM_ptr->text(LOG_ERROR, 1, "Can't send %s over TCP/IP to remote host %s on port number %d", WIDName(abortRq_ptr->getWidType()), getRemoteHostname(), getRemoteConnectPort());
		}
	}
...
//<<===========================================================================
{
	...
	if (!result)
	{
		if (loggerM_ptr)
		{
			loggerM_ptr->text(LOG_ERROR, 1, "Can't send %s over TCP/IP to remote host %s on port number %d", mapCommandName(command_ptr->getCommandId()), getRemoteHostname(), getRemoteConnectPort());
		}
	}
...
//<<===========================================================================
{
	...
	if (!result)
	{
		if (loggerM_ptr)
		{
			loggerM_ptr->text(LOG_ERROR, 1, "Can't send %s %s over TCP/IP to remote host %s on port number %d", mapCommandName(command_ptr->getCommandId()), dataset_ptr->getIodName(), getRemoteHostname(), getRemoteConnectPort());
		}
	}
	else
	{
		// store pointer to data in validationengine for validation purposes
		VALIDATION->setLastReqDataset(dataset_ptr);
	}
...
//<<===========================================================================
{
	...
	bool result = associationM.listen();
	if (!result)
	{
		if (loggerM_ptr)
		{
...
//<<===========================================================================
{
	...
	if (rxDataset_ptr)
	{
		...
		{
			RELATIONSHIP->analyseStorageDataset(rxDataset_ptr, loggerM_ptr);
		}
	}
	else
	{
...
//<<===========================================================================
{
	...
	result = VALIDATION->validate(command_ptr, refCommand_ptr, validationFlag, serializerM_ptr);
...
//<<===========================================================================
{
	...
	VALIDATION->setStrictValidation(strictValidation);
	VALIDATION->setIncludeType3NotPresentInResults(getIncludeType3NotPresentInResults());
	bool result1 = VALIDATION->validate(command_ptr, refCommand_ptr, validationFlag, serializerM_ptr);

	// validate the dataset - refDataset_ptr maybe NULL
	bool result2 = VALIDATION->validate(dataset_ptr, refDataset_ptr, command_ptr, validationFlag, serializerM_ptr, ae_session_ptr);
