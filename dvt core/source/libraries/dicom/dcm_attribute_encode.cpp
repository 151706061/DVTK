...
//<<===========================================================================
{
	...
	if (loggerM_ptr)
	{
		if (GetVR() == ATTR_VR_SQ) 
		{
			if (definedLengthM)
			{
				loggerM_ptr->text(LOG_DEBUG, 1, "Encode - Attribute (%04X,%04X), vr is %s, length is %08X", group, element, stringVr(GetVR()), length32);
			}
			else
			{
				loggerM_ptr->text(LOG_DEBUG, 1, "Encode - Attribute (%04X,%04X), vr is %s, length is %d, with undefined length (0xFFFFFFFF)", group, element, stringVr(GetVR()), length32);
			}
		}
		else
		{
			// check if we have to encode compressed pixel data
			if ((group == PIXEL_GROUP) && 
				(element == PIXEL_DATA) && 
				(dataTransfer.isCompressed())) 
			{
				loggerM_ptr->text(LOG_DEBUG, 1, "Encode - Attribute (%04X,%04X), vr is %s, length is undefined (0xFFFFFFFF)", group, element, stringVr(GetVR()));
			}
			else
			{
				loggerM_ptr->text(LOG_DEBUG, 1, "Encode - Attribute (%04X,%04X), vr is %s, length is %08X", group, element, stringVr(GetVR()), length32);
	...
	if ((GetVR() == ATTR_VR_SQ) &&
		(!definedLengthM))
	{
		// force length undefined
	    length32 = UNDEFINED_LENGTH;

	    if (loggerM_ptr)
	    {
	    	loggerM_ptr->text(LOG_DEBUG, 1, "Encode - forcing SQ length to UNDEFINED for attribute (%04X,%04X)", group, element);
	    }
	}
	...
	if (dataTransfer.isExplicitVR()) 
	{
			...
			(vr == ATTR_VR_UT)) 
		{
				...
				(dataTransfer.isCompressed())) 
			{
				if (loggerM_ptr)
				{
					loggerM_ptr->text(LOG_INFO, 1, "Encoding compressed Pixel Data with length: %08X", length32);
				}
				length32 = UNDEFINED_LENGTH;
			}
...
//<<===========================================================================
{
	...
	if (dataTransfer.isExplicitVR()) 
	{
			...
			(vr == ATTR_VR_UT)) 
		{
            ...
            {
    			// try to get VR by consulting Definitions
	    		ATTR_VR_ENUM def_vr = DEFINITION->GetAttributeVr(GetMappedElement(), GetMappedElement());
				if (def_vr != ATTR_VR_UN)
                {
				    ...
				    if (loggerM_ptr)
				    {
					    ...
					    loggerM_ptr->text(LOG_WARNING, 1, "Will use Little Endian byte ordering to decode the attribute value irrespective of the negotiated Transfer Syntax");
				    }
...
//<<===========================================================================
{
	...
	// if the incoming data has an uneven length it should be reported
	if ((length != UNDEFINED_LENGTH) &&
		(length & 0x01))
	{
		if (loggerM_ptr)
		{
			loggerM_ptr->text(LOG_ERROR, 1, "Decoded Attribute (%04X,%04X) has an uneven length: 0x%X=%d", group, element, length, length);
		}
	}
