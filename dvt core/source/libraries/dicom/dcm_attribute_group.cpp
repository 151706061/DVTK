...
//<<===========================================================================
{
	...
	while (mergeDicomObject_ptr->GetNrAttributes()) 
	{
		...
		DCM_ATTRIBUTE_CLASS *attribute_ptr = GetMappedAttribute(mergeAttribute_ptr->GetMappedGroup(), mergeAttribute_ptr->GetMappedElement());
...
DCM_ATTRIBUTE_CLASS *DCM_ATTRIBUTE_GROUP_CLASS::GetMappedAttribute(UINT16 group, UINT16 element)
...
{
	// call the base class
	return static_cast<DCM_ATTRIBUTE_CLASS*>(ATTRIBUTE_GROUP_CLASS::GetMappedAttribute(group, element));
}
