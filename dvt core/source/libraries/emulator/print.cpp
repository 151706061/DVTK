...
//<<===========================================================================
{
	...
	*dataset_ptr_ptr = NULL;
	...
	if (noImages == 0) 
	{
		if (loggerM_ptr)
		{
			loggerM_ptr->text(LOG_WARNING, 1, "Image log Format (2010,0010): %s unknown - add to Definition", imageDisplayFormat_ptr);
		}
	}
	...
	if (datasetM_ptr->getCSValue(TAG_ANNOTATION_DISPLAY_FORMAT_ID, annotationDisplayFormatId_ptr, CS_LENGTH))
	{
		...
		if (noBoxes == 0)
		{
			if (loggerM_ptr)
			{
				loggerM_ptr->text(LOG_WARNING, 1, "Annotation Display Format ID (2010,0030): %s unknown - add to Definition", annotationDisplayFormatId_ptr);
			}
		}
