...
bool DEF_ATTRIBUTE_GROUP_CLASS::EvaluateAddUserOptionalModule (ATTRIBUTE_GROUP_CLASS * actual_data)

//  DESCRIPTION     : Return true when the actual data contains an attribute
//                    in this module
//  PRECONDITIONS   :
//  POSTCONDITIONS  :
//  EXCEPTIONS      : 
...
//<<===========================================================================
{
    int     nr_attributes = this->attributesM.size ();
    bool    found = false;
    int     i = 0;
    while ((!found) && (i<nr_attributes))
    {
        ...
        { 
            // see if attribute is present in the attribute group
            if (actual_data->GetAttribute (group, element))
            {
                found = true;
