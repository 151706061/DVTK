...
//<<===========================================================================
{
    ...
    if (status)
    {
        SetModuleResultsFromDcm(dicomdirDataset_ptr, true);
    }

    if (refDataset_ptr)
    {
        SetModuleResultsFromDcm(refDataset_ptr, false);
    }
...
//<<===========================================================================
{
    ...
    if (recordDef_ptr != NULL)
    {
        ...
        // is loaded.
        CreateAttributeGroupResultsFromDef(recordDef_ptr, record_ptr, recordResult_ptr, false);
    }
    ...
    recordResult_ptr->Validate(directoryM);
    ...
    {
        recordResult_ptr->ValidateReferenceFile(uidLinksM_ptr, 
                validationFlag, 
                aeSession_ptr, 
                logger_ptr, 
                serializer_ptr, 
...
//<<===========================================================================
{
    ...
    if (status)
    {
        ...
        SetModuleResultsFromDcm(fileDataset_ptr->getFileMetaInformation(), true);
    ...
    {
		// Update the results structure with the source values.
        SetModuleResultsFromDcm(fileDataset_ptr->getDataset(), true);
...
//<<===========================================================================
{
        ...
        (uidInDataset_ptr))
    {
        ...
        {
            sprintf(message, 
                "UID mismatch with tag 0x%08lX in referenced file IOD", 
                TAG_SOP_CLASS_UID);
            uidInRecordAttr_ptr->GetMessages()->AddMessage(VAL_RULE_A_MEDIA_4, message);

            sprintf(message, 
                "UID mismatch with tag 0x%08lX in referring record", 
                TAG_REFERENCED_SOP_CLASS_UID_IN_FILE);
        ...
        (uidInDataset_ptr))
    {
        if (uidInFile_ptr->Compare(NULL, uidInDataset_ptr) != MSG_EQUAL)
        {
            ...
            sprintf(message, 
                "UID mismatch with tag 0x%08lX in referring file meta header", 
        ...
        (uidInDataset_ptr))
    {
        if (uidInRecord_ptr->Compare(NULL, uidInDataset_ptr) != MSG_EQUAL)
        {
            ...
            sprintf(message, 
                "UID mismatch with tag 0x%08lX in referring record", 
                TAG_REFERENCED_SOP_INSTANCE_UID_IN_FILE);
            uidInDatasetAttr_ptr->GetMessages()->AddMessage(VAL_RULE_A_MEDIA_4, message);
        }
    }
        ...
        (uidInRecord_ptr))
    {
        ...
        {
            sprintf(message, 
                "Transfer syntax mismatch with tag 0x%08lX in referring record", 
                TAG_REFERENCED_TRANSFER_SYNTAX_UID_IN_FILE);
            uidInFileAttr_ptr->GetMessages()->AddMessage(VAL_RULE_A_MEDIA_4, message);

            sprintf(message, 
                "Transfer syntax mismatch with tag 0x%08lX in referring file meta header", 
        ...
        if (dcmRecordAttr_ptr != NULL)
        {
                ...
                (!SkipAttribute(tag)))
            {
                ...
                if (valFileAttr_ptr == NULL)
                {
                    ...
                    {
                        valRecordAttr_ptr->GetMessages()->AddMessage(VAL_RULE_A_MEDIA_5,
                            "An attribute with this tag has not been found in the referred file");
                    }
                }
                else
                {
                    ...
                    if (recordVm > 0)
                    {
                        ...
                        if (recordVm != fileVm)
                        {
                            ...
                            sprintf(message,
                                "Value multiplicity mismatch %ld != %ld of attribute (0x%08lX) in referenced file",
                                recordVm, 
                                fileVm, 
                                tag);
                            valRecordAttr_ptr->GetMessages()->AddMessage(VAL_RULE_A_MEDIA_5, message);
                            sprintf(message,
                                "Value multiplicity mismatch %ld != %ld of attribute (0x%08lX) in referring record",
                                fileVm, 
                                recordVm, 
                                tag);
                        ...
                        else
                        {
                            ...
                            {
                                sprintf(message,
                                    "Value mismatch for attribute 0x%08lX with corresponding tag in referring record",
                                    tag);
...
//<<===========================================================================
{
    ...
    if ((offset != DICOMDIR_ILLEGAL_OFFSET) &&
        (offset != 0))
    {
        RECORD_LINK_CLASS *childRecord_ptr = GetRecordLinkStructByOffset(recordLink_ptr->GetDownLinkOffset());
        if (childRecord_ptr)
        {
        ...
        {
            recordLink_ptr->GetMessages()->AddMessage(VAL_RULE_A_MEDIA_7,
                "Downlink offset value points to a non-existing record.");
        }
    }
}
...
//<<===========================================================================
{
    ...
    if ((offset != DICOMDIR_ILLEGAL_OFFSET) &&
        (offset != 0))
    {
        RECORD_LINK_CLASS *childRecord_ptr = GetRecordLinkStructByOffset(recordLink_ptr->GetHorLinkOffset());
        if (childRecord_ptr)
        {
            // Since the record to be validated is on the same level
            // as the one we've currently checked, the parent is the same.
            CheckDirChain(childRecord_ptr, parentRecordTypeIndex);
        }
        else
        {
