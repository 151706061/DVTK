...
//<<===========================================================================
{
	...
	else
	{
		if (loggerM_ptr)
		{
			loggerM_ptr->text(LOG_ERROR, 1, "NETWORK_TF_CLASS::rewind() failed. Known DVT programming error.");
			loggerM_ptr->text(LOG_ERROR, 1, "A workaround is to increase the size of the session parameter DVT-MAXIMUM-LENGTH-RECEIVED.");
			loggerM_ptr->text(LOG_ERROR, 1, "This bug will be fixed in the next release. It is reproducable with a value of 16384 using file VASCSRTest1.dcm from the SCU Emulator.");
		}
		result = false;
	}
