	...
	while (fgets(line, MAXIMUM_LINE_LENGTH, fdM_ptr) != NULL) 
	{
		...
		if (length > 0) 
		{
			...
			for (int j = 0; j < count; j++) 
			{
				...
				string dicomScript(&line[index]);
				string scriptFilename = pathnameM + dicomScript;
				DICOM_SCRIPT_CLASS	*dicomScript_ptr  = NULL;
				FILE *fd_ptr = NULL;

				// check the existance of the script file name and also for invalid script file
				if((fd_ptr = fopen(scriptFilename.c_str(),"r")) != NULL)
				{
					fclose(fd_ptr);
					dicomScript_ptr = new DICOM_SCRIPT_CLASS(sessionM_ptr, dicomScript);
				}
				else
				{
					if (loggerM_ptr)
					{
						loggerM_ptr->text(LOG_ERROR, 1, "Invalid DICOMScript: %s, the DICOMScript doesn't exist", dicomScript.c_str());
					}
				}
			
				if (loggerM_ptr && dicomScript_ptr) 
				{
					if (onlyParseFileM)
					{
						loggerM_ptr->text(LOG_INFO, 1, "Parsing DICOMScript: %s  for %d of %d times", dicomScript.c_str(), j + 1, count);
					}
					else
					{
						loggerM_ptr->text(LOG_INFO, 1, "Executing DICOMScript: %s  for %d of %d times", dicomScript.c_str(), j + 1, count);
					}
				}

				// execute the contents
				if (dicomScript_ptr != NULL)
				{
					dicomScript_ptr->setInSuperScript(true);
					if (!dicomScript_ptr->execute()) 
					{
						if (loggerM_ptr)
						{
							loggerM_ptr->text(LOG_INFO, 1, "Failed to execute iteration %d of DICOMScript: %s", j + 1, dicomScript.c_str());
						}
						...
						{
							// clean-up and return
							delete dicomScript_ptr;
							return false;
						}
					}

					// free up the DICOMScript
					delete dicomScript_ptr;
				}
				else
				{
