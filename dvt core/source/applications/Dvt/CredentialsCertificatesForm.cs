...
namespace Dvt
{
	...
	public class CredentialsCertificatesForm : System.Windows.Forms.Form
	{
		...
		private ArrayList _KeysInformation = null;
		private string _ExceptionText = "";

		public CredentialsCertificatesForm(Dvtk.Sessions.Session session, bool show_credentials)
		{
            //
            ...
            if (session is Dvtk.Sessions.ISecure)
            {
                ...
                if (show_credentials)
                {
                    ...
                    _FileContainingKeys = (session as Dvtk.Sessions.ISecure).SecuritySettings.CredentialsFileName;
                    this._cred_handling = (session as Dvtk.Sessions.ISecure).CreateSecurityCredentialHandler ();
                }
                else
                {
                    this.Text = "SUT Public Keys (certificates)";
                    _FileContainingKeys = (session as Dvtk.Sessions.ISecure).SecuritySettings.CertificateFileName;
                    this._cert_handling = (session as Dvtk.Sessions.ISecure).CreateSecurityCertificateHandler ();
                }
		...
		private void InitializeComponent()
		{
			...
			this.ButtonAddCertificate.TabIndex = 4;
			...
			this.ButtonRemoveCertificate.TabIndex = 4;
			...
			this.DataGridCertificates.TabIndex = 5;
		...
		private void ButtonAddCertificate_Click(object sender, System.EventArgs e)
		{
			if (this._show_credentials)
			{
				...
				{
					this._cred_handling.ImportCredentialFromOtherFile (this.DialogAddCertificate.FileName, "");
        ...
        {
            if (this._show_credentials)
                (this._session as Dvtk.Sessions.ISecure).DisposeSecurityCredentialHandler ();
            else
                (this._session as Dvtk.Sessions.ISecure).DisposeSecurityCertificateHandler ();
        }
