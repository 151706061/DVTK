...
namespace Dvt {
    ...
    public class UserControlSessionTree : System.Windows.Forms.UserControl {
        ...
        private ProjectForm2 parentForm ;
        ...
        public ProjectForm2 ParentForm {
            get {
                return parentForm ;
            }
            set {
        ...
        private void OnEndExecution(object theSource, object theEvent) {
            ...
            if (theSessionTreeNodeToRefresh != null) {
                UpdateSessionNode(theSessionTreeNodeToRefresh,tempSession );
        ...
        private void OnStopExecution(object theSource, object theEvent) {
            if ( ((theEvent is StopExecution) && ( ((StopExecution)theEvent)._ProjectForm == parentForm)) ||
                (theEvent is StopAllExecution)
                ) {	
        ...
        private void OnSessionReplaced(object theSource, object theEvent) {
            ...
            UpdateSessionNode(theSessionNodeToUpdate, theSessionReplaced._NewSession);
...
//				else 
//				{	Object theTreeNodeTag = (Object)theSessionNode.Tag;
//					UpdateSessionNodeTextMainNodeOnly(theTreeNode, (Session)theTreeNodeTag);
//				}
            }
        }
        private void OnStartExecution(object theSource, object theEvent) {
            ...
            if (theSource == parentForm) {
               ...
               //  nodeInformation.StoreExpandInformation(theSessionTreeNode);
                ...
                if (theSessionTreeNodeToDisable != null) {
                    UpdateSessionNode(theSessionTreeNodeToDisable, tempSession);
        ...
        private void OnSessionChanged(object theSource, object theEvent) {
            ...
            if (theTreeNode != null) {
                if ( (theSessionChange.SessionChangeSubTyp == SessionChange.SessionChangeSubTypEnum.RESULTS_DIR) ||
                    ...
                    UpdateSessionNode(theTreeNode, theSessionChange.SessionApp);
                }
                else {
       ...
       public void UpdateTreeView() {
            ...
            for (int theIndex = 0; theIndex < projectApp.Sessions.Count ; theIndex++) {
                ...
                UpdateSessionNode(theTreeNode, session);
		...
		public TreeNode GetSelectedUserNode() 
		{
			...
			{
				TreeNode theprojectNode = userControlTreeView.SelectedNode;
				if ((theprojectNode = userControlTreeView.SelectedNode)!= null ) 
				{
        ...
        public DvtkApplicationLayer.Session  GetSession() {
			...
			else
			{
				...
				{
					Debug.Assert(true, "Not expected to get here.");
				}
			}
...
//        }
        ...
        public void ViewExpandedScriptFile() {
            Object selectedScriptFileTag = GetSelectedTag() as DvtkApplicationLayer.Script;

            if (selectedScriptFileTag != null) {
                if (((DvtkApplicationLayer.Script)selectedScriptFileTag).ScriptFileName.ToLower().EndsWith(".vbs")) {
                    // TODO!!!!!
                    ...
                    //                        new DvtkApplicationLayer.VisualBasicScript((((DvtkApplicationLayer.Script)selectedScriptFileTag).ParentSession.Implementation.sc , ((DvtkApplicationLayer.Script)selectedScriptFileTag).ScriptFileName);
                    //
        ...
        public void GenerateDICOMDIR() {
            ...
            if (theOpenFileDialog.ShowDialog() == DialogResult.OK) {
                ...
                theMediaSession.Implementation.StartResultsGathering("CREATEDDICOMDIR_res.xml");
                theMediaSession.MediaSessionImplementation.GenerateDICOMDIR(dcmFiles);
                theMediaSession.Implementation.EndResultsGathering();
        ...
        public TreeNode ExpandAndSelectResultFile(TreeNode theTreeNode, string theResultsFileName) {
            ...
            foreach(TreeNode theSubTreeNode in theTreeNode.Nodes) {
                if (theSubTreeNode.Tag is Result) {
                    ...
                    //if (theResultsFileTag.ContainsResultsFile(theResultsFileName)){
		...
		public void SearchAndSelectResultNode(string theDirectory, string theResultsFileName) 
		{
			...
			foreach(TreeNode theProjectNode in userControlTreeView.Nodes) 
			{
				...
				foreach (TreeNode theSessionNode in theProjectNode.Nodes ) 
				{
					...
					if(theSessionTag.ResultsRootDirectory == theDirectory) 
					{
        ...
        private void userControlTreeView_BeforeSelect(object sender, System.Windows.Forms.TreeViewCancelEventArgs e) {
            ...
            if (parentForm.TabControl.SelectedTab == parentForm.TabSessionInformation ) {
                //TextBoxSessionTitle.Focus();
                //TextBoxDVTAETitle.Focus();
                //TextBoxSUTAETitle.Focus();

                parentForm.TabControl.SelectedTab.Focus();
            }
        }
    }
}
