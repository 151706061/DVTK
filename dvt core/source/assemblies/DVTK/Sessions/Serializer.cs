...
namespace Dvtk.Sessions
{
        ...
        : Wrappers.ISerializationTarget
    {
            ...
            System.String message)
        {
            ...
            applicationActivityReport.DvtDetailToXml(this.m_serializationWriter.DetailStreamWriter, 0);

            // stream application activity to summary output
            applicationActivityReport.DvtSummaryToXml(this.m_serializationWriter.SummaryStreamWriter, 0);
        }
            ...
            System.String message)
        {
            ...
            userActivityReport.Message = DvtToXml.ConvertString(message);
            userActivityReport.Level = _Convert(activityReportLevel);

            // stream user activity to detailed output
            userActivityReport.DvtDetailToXml(m_serializationWriter.DetailStreamWriter, 0);

            // stream user activity to summary output
            userActivityReport.DvtSummaryToXml(m_serializationWriter.SummaryStreamWriter, 0);
        }
			...
			System.String message)
		{
			...
			validationMessage.Message = DvtToXml.ConvertString(message);
...
//			if (validationMessage.Type != DvtkData.Validation.MessageType.Info)
//			{
